...
/*
TODO the candidate for removal
enum _ID_Out{
	_id_OUT1			= 0,
	_id_OUT12,
	_id_OUT2,
...
class io_buffer{
	public:
		io_buffer( size_t nl ) : Buffer( NULL ), NL( nl ), Size( 0 ){};
...
class _Grid_Key{
	public:
		_Grid_Key( void ){};
		_Grid_Key( const string &path, const string &name ) : Path( path ), Name( name ){};
		_Grid_Key( const _Grid_Key &tabkey ) : Path( tabkey.Path ), Name( tabkey.Name ){};
		~_Grid_Key( void ){};
	public:
		...
		{
			if( Path < tabkey.Path ){ return true; }
			if( Path == tabkey.Path && Name < tabkey.Name ){ return true; }
			return false;
		};
	public:
		string Path;
...
class _Grid_Element{
	public:
		_Grid_Element( void ) : IsStyle( false ), Type( GRID_NONE )
		{
			Position = make_pair( -1, -1 );
		};
		_Grid_Element( const string &data, int type, const pair<int,int> &position, char delim = '\t' ) 
			: IsStyle( false ), Type( type ), InitStr( data ), Position( position )
		{
			Data = ( Type&GRID_COMBO )? data.substr( 0, data.find( delim, 0 )): data;
		};
		_Grid_Element( const _Grid_Element &x ) : IsStyle( x.IsStyle ), Type( x.Type ), Data( x.Data ), InitStr( x.InitStr ), Position( x.Position ){};
		~_Grid_Element( void ){};
	public:
...
class t_data{
	public:
		t_data( void ) : Value( 0. ), Variance( 0. ){};
		t_data( double data ) : Value( data ), Variance( 0. ){};
		t_data( double data, double var ) : Value( data ), Variance( var ){};
		~t_data( void ){};
	public:
...
{
	s << x.Value;
	if( x.Variance >= 0.0 ){ s << "(" << x.Variance << ")"; }
	return s;
}
...
class hhn_pair{
	...
	public:
		...
		bool operator < ( const hhn_pair<T> &a ) const{ return X < a.X; };
	public:
		T X;
		T Y;
	};
