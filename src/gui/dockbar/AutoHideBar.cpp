...
CAutoHideBar::CAutoHideBar()
{
	...
	if( !(::GetClassInfo( hInst, _T( LPCTSTR( "AUTO_HIDE_BAR" )), &wndcls ))){
...
//		wndcls.hbrBackground    = (HBRUSH)( COLOR_WINDOWTEXT+1);

		wndcls.lpszMenuName     = NULL;
		wndcls.lpszClassName    = _T( "AUTO_HIDE_BAR" );
	...
	m_Height = 30;
	m_pImageList = NULL;
	m_Font.CreateFont(20,0, 0,0, FW_NORMAL, 0,0,0, DEFAULT_CHARSET,
		OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
		CLEARTYPE_QUALITY, FF_ROMAN , "Tahoma");
	m_VertFont.CreateFont(20,0, -900,-900, FW_NORMAL, 0,0,0, DEFAULT_CHARSET,
		OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
		CLEARTYPE_QUALITY, FF_ROMAN , "Tahoma");
}
...
{
	m_Style = Style;
	CRect rt(10, 10, 110, 110);
	return CWnd::Create(_T( "AUTO_HIDE_BAR" ), "AutoHideBar", 
					WS_CHILD | WS_CLIPCHILDREN, rt, pWnd, 
					233 + Style);
}
		...
		if(m_ButtonInfo[idx]->m_Style & (DRAWBTNSTYLE_BTN | DRAWBTNSTYLE_GROUP))
		{
			if(m_pImageList && m_ButtonInfo[idx]->m_ImgIndex >= 0)
			{
				...
				if (m_pImageList->GetImageInfo(m_ButtonInfo[idx]->m_ImgIndex, &Info))
				{
					...
					if(bHorz)
						pt.y = m_ButtonInfo[idx]->m_Rect.top + 6;
			...
			if(bHorz)
				pDC->ExtTextOut(m_ButtonInfo[idx]->m_Rect.left + 2 + 16,
					m_ButtonInfo[idx]->m_Rect.top+5,
					ETO_CLIPPED,
					NULL, m_ButtonInfo[idx]->m_Name, NULL);
			else
				pDC->ExtTextOut(m_ButtonInfo[idx]->m_Rect.left+25,
					m_ButtonInfo[idx]->m_Rect.top + 18,
					ETO_CLIPPED,
					NULL, m_ButtonInfo[idx]->m_Name, NULL);
...
void CAutoHideBar::AddButton(LPCSTR pName, int Style, CWnd * pWnd, CWnd * pOrginalParent, CRect* pOrginalRect, CPinDockBar * pControlBar, int ImgIndex)
{
	...
	if(!(Style & DRAWBTNSTYLE_SEP))
	{
		...
		if(!pBtn->m_Frame.CreateEx(WS_EX_LEFT, _T( "DRAW_FRAME_WND" ), NULL, 
							(WS_POPUP | WS_BORDER | WS_THICKFRAME | WS_CLIPCHILDREN ), rt, AfxGetMainWnd(), 0))
			AfxThrowResourceException();
//		pBtn->m_Frame.SetParent( this );
...
LRESULT CAutoHideBar::OnSizeParent(WPARAM, LPARAM lp)
{
	...
	switch(m_Style)
    {
		case LEFTSIDE:
			...
			MoveWindow(pParams->rect.left - height, pParams->rect.top, height, rt_height);
			pParams->sizeTotal.cx -= height;
			break;

		case RIGHTSIDE:
			pParams->rect.right -= height;
			MoveWindow(pParams->rect.right , pParams->rect.top, height, rt_height);
			pParams->sizeTotal.cx -= height;
			break;

		case TOPSIDE:
			pParams->rect.top += height;
			MoveWindow(pParams->rect.left, pParams->rect.top - height, rt_width, height);
			pParams->sizeTotal.cy -= height;
			break;

		case BOTTOMSIDE:
			pParams->rect.bottom -= height;
			MoveWindow(pParams->rect.left, pParams->rect.bottom, rt_width, height);
			pParams->sizeTotal.cy -= height;
			break;
