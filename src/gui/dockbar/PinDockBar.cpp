...
void CSCBPushPinButton::Paint(CDC* pDC)
{
    ...
    if (bPushed)
        pDC->Draw3dRect(rc, ::GetSysColor(COLOR_3DDKSHADOW),
            ::GetSysColor(COLOR_3DHIGHLIGHT));
    else
        if (bRaised)
            pDC->Draw3dRect(rc, ::GetSysColor(COLOR_3DHIGHLIGHT),
    ...
    int pointsize = MulDiv(120, 96, ppi); // 6 points at 96 ppi
    font.CreatePointFont(pointsize, _T("Marlett"));
    CFont* oldfont = pDC->SelectObject(&font);

    pDC->TextOut(ptOrg.x + 1, ptOrg.y + 1, CString(_T("v"))); // < >-like
...
void CPinDockBar::NcCalcClient(LPRECT pRc, UINT nDockBarID)
{
    ...
    CPoint ptOrgBtn;
    if (bHorz)
        ptOrgBtn = CPoint(rc.left - 19, rc.top+2);
    else
        ptOrgBtn = CPoint(rc.right - 16, rc.top - 18);

    m_biHide.Move(ptOrgBtn - rcBar.TopLeft());
	if(!IsFloating())
	{
	    if (bHorz)
		    m_PushPin.Move(CPoint(rc.left - 18, rc.bottom - 18) - rcBar.TopLeft());
	    else
			m_PushPin.Move(CPoint(rc.left+1, rc.top - 18) - rcBar.TopLeft());
	}
    *pRc = rc;
}
...
// add pushpin
    ...
    if (bFont)
    {
        ...
        CPoint ptOrg = bHorz ?
            CPoint(rcGrip.left +22, rcGrip.top+8) :
            CPoint(rcGrip.left + 4, rcGrip.top );
