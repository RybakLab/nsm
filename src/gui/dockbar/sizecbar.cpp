...
CSizingControlBar::CSizingControlBar()
{
 ...
 if( !(::GetClassInfo( hInst, _T( LPCTSTR( "SIZING_CONTROL_BAR" )), &wndcls ))){
     // otherwise we need to register a new class
     wndcls.style            = 0; // CS_DBLCLKS;
     ...
     wndcls.lpszClassName    = _T( "SIZING_CONTROL_BAR" );
     if( !AfxRegisterClass( &wndcls )){
         AfxThrowResourceException();
         }
     }

    m_szMinHorz = CSize(400, 400);
    m_szMinVert = CSize(400, 400);
    m_szMinFloat = CSize(400, 400);
    m_szHorz = CSize(400, 400);
    m_szVert = CSize(400, 400);
    m_szFloat = CSize(400, 400);
    m_bTracking = FALSE;
    m_bKeepSize = FALSE;
                               ...
                               DWORD dwStyle)
{
    ...
    if (!CWnd::Create(_T( "SIZING_CONTROL_BAR" ), lpszWindowName, dwStyle,
    ...
    if (m_dwSCBStyle & SCBS_SHOWEDGES)
    {
        ...
        for (int i = 0; i < 4; i++)
            if (GetEdgeRect(rcBar, GetEdgeHTCode(i), rcEdge))
                mdc.Draw3dRect(rcEdge, ::GetSysColor(COLOR_3DFACE),
