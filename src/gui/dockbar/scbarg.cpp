...
BOOL CSizingControlBarG::OnEraseBkgnd( CDC* pDC )
{
...
void CSizingControlBarG::NcCalcClient(LPRECT pRc, UINT nDockBarID)
{
    ...
    CPoint ptOrgBtn;
    if (bHorz)
        ptOrgBtn = CPoint(rc.left - 20, rc.top);
    else
        ptOrgBtn = CPoint(rc.right - 14, rc.top - 18);
...
void CSizingControlBarG::NcPaintGripper(CDC* pDC, CRect rcClient)
{
    ...
    pDC->Draw3dRect(gripper, ::GetSysColor(COLOR_MENUHILIGHT),
...
void CSCBButton::Paint(CDC* pDC)
{
    ...
    if (bPushed)
        ...
        if (bRaised)
            pDC->Draw3dRect(rc, ::GetSysColor(COLOR_MENUHILIGHT),
                ::GetSysColor(COLOR_BTNSHADOW));
    ...
    pDC->TextOut(ptOrg.x + 1, ptOrg.y + 1, CString(_T("r"))); // x-like
