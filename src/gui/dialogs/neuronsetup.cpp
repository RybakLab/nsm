...
BEGIN_MESSAGE_MAP(CPopulatProperties, CDialog)
	...
	ON_MESSAGE( GUI_DEL_UNITS, OnDelUnits )
END_MESSAGE_MAP()
...
void CPopulatProperties::add_unit( const CPoint &point )
{
	...
	if( NeuronView.GetActiveBranch( path )){
		RefreshUnusedUnits( path.c_str() );
		HMENU hMenu = ::CreatePopupMenu();
		if( !UnitNames.empty() && NULL != hMenu ){
			for( size_t i = 0; i < UnitNames.size(); ++i )
				::AppendMenu( hMenu, MF_STRING, i+1, UnitNames[i].c_str() );
			CMenu menu;
			menu.Attach( hMenu );
...
void CPopulatProperties::RefreshUnusedUnits( const char *full_path )
{
	UnitNames.clear();
	PopulateTemplate.get_unusednames( full_path, UnitNames );
}
...
bool CPopulatProperties::DisplayUnit( uni_template *unit, string &start_path, bool collapse )
{
	if( unit ){
...
bool CPopulatProperties::DisplayUnitPar( uni_template *unit, string &start_path )
{
...
bool CPopulatProperties::DisplayUnitChildren( uni_template *unit, string &start_path, bool collapse )
{
