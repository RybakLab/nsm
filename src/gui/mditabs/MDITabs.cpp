...
afx_msg LRESULT CMDITabs::OnSizeParent(WPARAM, LPARAM lParam)
{
  ...
  {  
    ...
    if (m_bTop)
    {
      ...
      MoveWindow(pParams->rect.left, pParams->rect.top - height, m_width, m_height);
    }
    else
    {
      pParams->rect.bottom -= height;
      MoveWindow(pParams->rect.left, pParams->rect.bottom- offset, m_width, m_height);
    }
...
//    ::ExcludeClipRect(dc, 0, 0, m_width, 0); 
//    ::ExcludeClipRect(dc, m_width - 6, 1, m_width - 2, 0);
  }
...
/*
//		kakaja-to lazha s opredeleniem razmera? okna
		if (::IsZoomed(hWnd)!=0){
			CRect rect;
